...
exports.run = async (client, message, args) => {
  ...
  .setColor('DARK')  
    ...
    var canal = db.set(`idsugestao_${message.guild.id}`, args.join(" ").trim())
    if (isNaN(args.join(' '))) return message.channel.send(erro)
    
    message.channel.send(`<:check:832445897207447572> **»** Feito com sucesso. Agora, uma sugestão sera enviada no canal: <#${canal}>.`)
